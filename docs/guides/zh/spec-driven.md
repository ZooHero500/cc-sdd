...
本文档说明 cc-sdd 如何在 AI 驱动开发生命周期（AI-DLC）内实现规格驱动开发（SDD）。在决定运行哪条斜杠命令、审查哪份产出物，以及如何将工作流适配到团队时，可将本文档作为参考。
...
- **模板** — 调整 `{{KIRO_DIR}}/settings/templates/{requirements,design,tasks}.md` 以反映您的审查流程。cc-sdd 会将这些文件复制到每个规格目录中。
