...
## Tool Guidance
- **Conversation parsing**: Extract `/yy-spec-impl` patterns from history
...
### Error Scenarios
- **No Implementation Found**: If no `/yy-spec-impl` in history and no `[x]` tasks, report "No implementations detected"
...
- Re-run `/yy-spec-impl <feature> [tasks]` for fixes
- Re-validate with `/yy-validate-impl [feature] [tasks]`
