...
### Phase 2-4 Tools
- **SlashCommand**: Execute `/yy-spec-requirements`, `/yy-spec-design`, `/yy-spec-tasks`
...
- `/yy-validate-gap` - Gap analysis (integration check)
- `/yy-validate-design` - Design review (architecture validation)

## Next Steps:
1. Review generated specs (especially design.md)
2. Optional validation:
   - `/yy-validate-gap {feature}` - Check integration with existing codebase
   - `/yy-validate-design {feature}` - Verify architecture quality
3. Start implementation: `/yy-spec-impl {feature}`

## Note:
For complex features (integrations, security, APIs), use standard workflow:
/yy-spec-init → /yy-spec-requirements → /yy-validate-gap
→ /yy-spec-design → /yy-validate-design → /yy-spec-tasks
```
...
- Suggest: "Continue manually from `/yy-spec-{next-phase} {feature}`"
